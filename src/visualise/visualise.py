...
def draw_agent_trail(img, trail_data, rgb):
...
def show_keypress(img, action_x, action_y):
    ...
    if   action_x == 2 and action_y == 9    : x1 = x - (key_width / 2);                       x2 = x + (key_width / 2) + (1 * key_width)
    elif action_x == 2 and action_y == 10   : x1 = x - (key_width / 2) - (1 * key_width);     x2 = x + (key_width / 2) 
        
    # keypress for space
    elif action_x == 3 and action_y == 3    : x1 = x - (key_width / 2);                        x2 = x + (key_width / 2) + (4 * key_width)
    elif action_x == 3 and action_y == 4    : x1 = x - (key_width / 2) - (1 * key_width);      x2 = x + (key_width / 2) + (3 * key_width)
    elif action_x == 3 and action_y == 5    : x1 = x - (key_width / 2) - (2 * key_width);      x2 = x + (key_width / 2) + (2 * key_width)
    elif action_x == 3 and action_y == 6    : x1 = x - (key_width / 2) - (3 * key_width);      x2 = x + (key_width / 2) + (1 * key_width)
    elif action_x == 3 and action_y == 7    : x1 = x - (key_width / 2) - (4 * key_width);      x2 = x + (key_width / 2)

    # keypress for enter
    elif action_x == 3 and action_y == 8    : x1 = x - (key_width / 2);                        x2 = x + (key_width / 2) + (2 * key_width)
    elif action_x == 3 and action_y == 9    : x1 = x - (key_width / 2) - (1 * key_width);      x2 = x + (key_width / 2) + (1 * key_width)
    elif action_x == 3 and action_y == 10   : x1 = x - (key_width / 2) - (2 * key_width);      x2 = x + (key_width / 2)
    
    # other keypresses
    else                                    : x1 = x - (key_width / 2);                        x2 = x + (key_width / 2)

    keypress_img = cv2.rectangle(img, (int(x1), int(y-key_height/2)), (int(x2), int(y+key_height/2)), (80,80,80), -1)
...
def add_details(img, screen_img, text, has_vision, has_finger, model_time) :
    ...
    cv2.putText(img, "Trial time = " + str(model_time) + "ms",(500,150),cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,0),3,cv2.LINE_AA)
    if has_vision:
        cv2.putText(img,'Eye',(30,440), cv2.FONT_HERSHEY_SIMPLEX,2,(0,0,0),3,cv2.LINE_AA)
        cv2.circle(img,(440,420),40,eye_rgb,-1)
    if has_finger:
...
def visualise_agent(has_vision, has_finger, vision_file, finger_file, output_file):
    ...
    if has_finger:
        ...
        finger_data = interp_test_data(finger_data) 
    ...
    for i in range(data_size):
        ...
        if has_finger:
            ...
            img = draw_agent_trail(img, finger_trail, finger_rgb)
        ...
        if has_vision:
            ...
            img = draw_agent_trail(img,eye_trail, eye_rgb)
